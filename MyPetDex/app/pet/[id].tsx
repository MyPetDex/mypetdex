...
.header { background:#4486F4; padding:20px 24px; color:white; }
.header h1 { font-size:24px; font-weight:900; }
.header p { font-size:12px; opacity:0.75; margin-top:3px; }
.hero { display:flex; align-items:center; gap:16px; padding:16px 24px; border-bottom:1px solid #E2E8F0; }
.avatar { width:72px; height:72px; border-radius:36px; background:#E2E8F0; display:flex; align-items:center; justify-content:center; font-size:36px; border:3px solid #4486F4; overflow:hidden; }
.avatar img { width:72px; height:72px; object-fit:cover; }
.pet-name { font-size:22px; font-weight:800; }
.pet-meta { font-size:13px; color:#64748B; margin-top:2px; }
.section { padding:14px 24px; border-bottom:1px solid #E2E8F0; page-break-inside:avoid; break-inside:avoid; }
...
.med { background:#F8FAFC; border-radius:8px; padding:9px 12px; margin-bottom:6px; page-break-inside:avoid; break-inside:avoid; }
...
.footer { padding:14px 24px; text-align:center; background:#F8FAFC; page-break-before:avoid; break-before:avoid; page-break-inside:avoid; break-inside:avoid; margin-top:8px; }
.footer-brand { font-size:13px; font-weight:800; color:#4486F4; }
...
.header p { font-size:12px; opacity:0.75; margin-top:3px; }
.section { padding:14px 24px; border-bottom:1px solid #E2E8F0; page-break-inside:avoid; break-inside:avoid; }
.label { font-size:11px; font-weight:700; color:#4486F4; text-transform:uppercase; letter-spacing:0.8px; margin-bottom:10px; }
.desc { font-size:14px; color:#475569; line-height:20px; margin-bottom:8px; }
.serving { font-size:13px; color:#64748B; font-style:italic; }
.item { font-size:14px; color:#1E293B; margin-bottom:5px; padding-left:12px; }
.step { font-size:14px; color:#1E293B; margin-bottom:6px; line-height:20px; }
.nutrition { font-size:12px; color:#64748B; line-height:18px; }
.breed { font-size:13px; color:#475569; font-style:italic; }
.warning { font-size:12px; color:#EF4444; line-height:17px; }
.footer { padding:14px 24px; text-align:center; background:#F8FAFC; page-break-before:avoid; break-before:avoid; margin-top:8px; }
.footer-brand { font-size:13px; font-weight:800; color:#4486F4; }
.footer-sub { font-size:11px; color:#94A3B8; margin-top:2px; }
</style></head><body>
<div class="header"><h1>${recipe.name}</h1><p>Custom recipe for ${pet.name} · Generated by MyPetDex · ${today}</p></div>
<div class="section">
  <div class="label">About</div>
  <div class="desc">${recipe.description || ""}</div>
  <div class="serving">${recipe.servingInfo || ""}</div>
</div>
<div class="section">
  <div class="label">Ingredients</div>
  ${(recipe.ingredients || []).map((i: string) => `<div class="item">• ${i}</div>`).join("")}
</div>
<div class="section">
  <div class="label">Instructions</div>
  ${(recipe.instructions || []).map((s: string, idx: number) => `<div class="step">${idx + 1}. ${s}</div>`).join("")}
</div>
<div class="section">
  <div class="label">Daily Supplements</div>
  ${(recipe.supplements || []).map((s: string) => `<div class="item">• ${s}</div>`).join("")}
  ${recipe.multivitamin ? `<div class="item">• ${recipe.multivitamin}</div>` : ""}
</div>
<div class="section">
...
<div class="footer">
  ...
  <div class="footer-sub">The complete home for pet owners · home.mypetdex.app</div>
      ...
      const { uri } = await Print.printToFileAsync({ html, base64: false });
      await Sharing.shareAsync(uri, { mimeType: "application/pdf", dialogTitle: `${pet.name}'s Recipe` });
    } catch {
      // Fallback to plain text if PDF fails
      Share.share({ message: `${recipe.name} — Recipe for ${pet.name}\n\n${recipe.description}`, title: `${pet.name}'s Recipe` });
    }
  }
